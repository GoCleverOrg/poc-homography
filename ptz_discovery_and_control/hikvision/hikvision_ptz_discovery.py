...
CAMERAS = [{"ip": "10.207.99.178", "name": "Camera 1"}, {"ip": "10.237.100.15", "name": "Camera 2"}]
...
class HikvisionPTZ:
    ...
    def _make_request(self, method: str, endpoint: str, data: str | None = None) -> tuple[int, str]:
    ...
    def discover_endpoints(self) -> dict[str, tuple[int, str]]:
        ...
        for endpoint, method in endpoints_to_test:
            ...
            if status_code == 200:
                ...
                print("  ⚠ Bad Request (400) - Endpoint exists but needs valid data")
...
<PTZData>
    ...
    def test_movement_with_polling(
        self,
        pan: int = 0,
        tilt: int = 0,
        zoom: int = 0,
        ...
        if initial:
            status_history.append({"time": 0, "phase": "initial", **initial})
        ...
        while time.time() - start_time < duration:
            ...
            if status:
                status_history.append({"time": elapsed, "phase": "moving", **status})
        ...
        for i in range(max_stable_checks):
            ...
            if status:
                elapsed = time.time() - start_time
                status_history.append({"time": elapsed, "phase": "stabilizing", **status})
                pan_str = f"{status['pan']:.1f}°" if status["pan"] is not None else "N/A"
                tilt_str = f"{status['tilt']:.1f}°" if status["tilt"] is not None else "N/A"
                zoom_str = f"{status['zoom']:.1f}" if status["zoom"] is not None else "N/A"
                print(f"  [{elapsed:.2f}s] Pan: {pan_str}, Tilt: {tilt_str}, Zoom: {zoom_str}")
                ...
                if last_position:
                    pan_stable = (
                        status["pan"] is None
                        or last_position["pan"] is None
                        or abs(status["pan"] - last_position["pan"]) < 0.1
                    )
                    tilt_stable = (
                        status["tilt"] is None
                        or last_position["tilt"] is None
                        or abs(status["tilt"] - last_position["tilt"]) < 0.1
                    )
                    zoom_stable = (
                        status["zoom"] is None
                        or last_position["zoom"] is None
                        or abs(status["zoom"] - last_position["zoom"]) < 0.1
                    )
        ...
        if final:
            ...
            if initial and final["pan"] is not None and initial["pan"] is not None:
                ...
                delta_tilt = (
                    final["tilt"] - initial["tilt"] if final["tilt"] and initial["tilt"] else 0
                )
                delta_zoom = (
                    final["zoom"] - initial["zoom"] if final["zoom"] and initial["zoom"] else 0
                )
                print(
                    f"Total change - Pan: {delta_pan:+.1f}°, Tilt: {delta_tilt:+.1f}°, Zoom: {delta_zoom:+.1f}"
                )
...
def main():
    ...
    for cam_config in CAMERAS:
        camera = HikvisionPTZ(
            ip=cam_config["ip"], username=USERNAME, password=PASSWORD, name=cam_config["name"]
        )
            ...
            json.dump(
                {
                    ...
                    "ip": camera.ip,
                    "endpoints": {
                        k: {"status": v[0], "response": v[1]} for k, v in endpoints.items()
                    },
