...
def test_prompts(image_path: str, api_key: str, output_dir: str = None) -> list:
    ...
    for i, prompt in enumerate(PROMPTS_TO_TEST, 1):
        ...
        try:
            ...
            mask, total_polygons, confidence_scores = create_mask_from_response(api_response, frame.shape)
            ...
            print(f"   Detections: {result['detections']}, Polygons: {result['polygons']}, Coverage: {result['coverage']:.2f}%")
...
def print_results_table(results: list):
    ...
    for result in results:
        ...
        print(f"| {result['prompt']} | {result['detections']} | {result['polygons']} | {result['coverage']:.2f} | {result['avg_confidence']:.3f} | {quality_str} |")
...
def main():
    ...
    parser.add_argument('--output-dir', '-o', default='./prompt_test_results',
