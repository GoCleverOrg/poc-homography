...
def load_map_points(map_points_path: str | Path) -> GCPRegistry:
    """
    Load map points from JSON or YAML file using GCPRegistry.

    Args:
        map_points_path: Path to map points file (.json or .yaml/.yml)

    Returns:
        GCPRegistry containing loaded map points
...
def convert_map_points_to_list(registry: GCPRegistry) -> list[dict[str, Any]]:
    """
    Convert GCPRegistry to list format for web interface.

    Args:
        registry: GCPRegistry to convert
