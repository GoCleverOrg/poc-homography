"""SAM3 prompt testing CLI commands."""
...
from poc_homography.application.services import Sam3PromptResult
from poc_homography.application.services.sam3_testing_service import DEFAULT_PROMPTS
...
def sam3_command(
    ...
    # Check for API key / service availability
    if ctx.sam3_testing_service is None:
        typer.echo(
    ...
    # Run tests
    try:
        results = ctx.sam3_testing_service.test_prompts(
            image_path=image,
            prompts=None,  # Use default prompts
...
def _print_results_table(results: list[Sam3PromptResult]) -> None:
...
def _save_results_json(results: list[Sam3PromptResult], output_path: Path) -> None:
    """Save results to JSON file."""
    output = {
        "timestamp": datetime.now().isoformat(),
        "prompts_tested": DEFAULT_PROMPTS,
        "results": [
            {
                "prompt": r.prompt,
