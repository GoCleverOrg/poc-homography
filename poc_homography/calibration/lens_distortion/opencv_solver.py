...
class AnnotatedLineSolver:
    ...
    def solve(
        ...
        if not isinstance(intrinsic_matrix, np.ndarray) or intrinsic_matrix.shape != (3, 3):
            return self._fail("intrinsic_matrix must be a 3x3 numpy array")
    ...
    def _fail(message: str) -> SolverResult:
        logger.warning("Solver failed: %s", message)
        return SolverResult(
    ...
    def _compute_validation_rmse(
        ...
        overall_rmse = float(np.sqrt(total_error / total_samples))
