...
class GPSPositionMixin:
    ...
    This mixin automatically initializes GPS position attributes to None.
    Consuming classes can use set_camera_gps_position() to set the GPS reference point.

    Usage:
        class MyHomography(GPSPositionMixin, HomographyProvider):
            def __init__(self, ...):
                # No need to manually initialize _camera_gps_lat/_camera_gps_lon
                # The mixin handles initialization automatically
                ...

    Attributes:
        _camera_gps_lat: Camera GPS latitude (None until set via set_camera_gps_position)
        _camera_gps_lon: Camera GPS longitude (None until set via set_camera_gps_position)

    Note:
        This mixin uses __init_subclass__ to ensure attributes are properly initialized
        even if the consuming class doesn't call super().__init__().
    """
    _camera_gps_lat: Optional[float] = None
    _camera_gps_lon: Optional[float] = None

    def __init_subclass__(cls, **kwargs):
        """
        Ensure GPS attributes exist on subclasses.

        This is called automatically when a class inherits from GPSPositionMixin,
        ensuring that the GPS position attributes are properly set up.
        """
        super().__init_subclass__(**kwargs)
        # Set default values at class level if not already defined
        if not hasattr(cls, '_camera_gps_lat'):
            cls._camera_gps_lat = None
        if not hasattr(cls, '_camera_gps_lon'):
            cls._camera_gps_lon = None

    def __init__(self, *args, **kwargs):
        """
        Initialize GPS position attributes to None.

        This ensures instance-level initialization of GPS attributes.
        Subclasses should call super().__init__() to maintain compatibility,
        but this is optional as __init_subclass__ provides a safety net.
        """
        super().__init__(*args, **kwargs)
        # Initialize instance attributes (shadowing class attributes)
        if not hasattr(self, '_camera_gps_lat'):
            self._camera_gps_lat = None
        if not hasattr(self, '_camera_gps_lon'):
            self._camera_gps_lon = None
...
class HomographyProviderExtended(HomographyProvider):
    ...
    def get_pixels_per_meter(self) -> float:
        ...
        This method provides access to the pixels-per-meter scale factor
        used for converting metric world coordinates to pixel coordinates
        in map visualizations.

        Returns:
            float: Scale factor in pixels per meter. For example, a value of 100.0
                means that 1 meter in world space corresponds to 100 pixels in
                the map visualization.

        Note:
            This is a visualization parameter and does not affect the accuracy
            of homography projections, which work in metric coordinates.
        """
        pass
