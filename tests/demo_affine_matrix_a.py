...
def demo_compute_a_matrix():
    ...
    print("\nGeoTIFF Parameters:")
    print(f"  Pixel size: ({geotiff_params['pixel_size_x']}, {geotiff_params['pixel_size_y']}) m/pixel")
    print(f"  Origin: ({geotiff_params['origin_easting']}, {geotiff_params['origin_northing']}) UTM")
...
def demo_transform_reference_to_world():
    ...
    for px, py in reference_pixels:
        ...
        print(f"({px:3d}, {py:3d})      ({x_world:9.1f}, {y_world:9.1f})       ({dx:6.1f}m E, {dy:6.1f}m N)")
...
def demo_backward_compatibility():
    ...
    print(f"A matrix after None parameters:")
    print(geo.A)
    print(f"Is identity: {np.allclose(geo.A, np.eye(3))}")
