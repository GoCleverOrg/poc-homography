...
def sample_calibration_result():
    ...
    per_gcp_errors = [
        1.2, 2.3, 0.8, 1.5, 3.1,  # GCPs 0-4
        1.8, 2.0, 1.1, 2.8, 1.4,  # GCPs 5-9
        0.9, 1.6, 2.2, 1.3, 2.5,  # GCPs 10-14
        1.7, 0.7, 2.9, 1.0, 2.1   # GCPs 15-19
    ]
...
def sample_gcps_for_plot():
    ...
    for i in range(20):
        ...
        gcps.append({
            'gps': {
                'latitude': ref_lat + row * 0.0002,
                'longitude': ref_lon + col * 0.0002
            },
            'image': {
...
class TestTrainTestSplit:
    ...
    def test_calibrator_validation_split_out_of_range_high(self, real_camera_geometry, valid_gcps_large):
    ...
    def test_train_test_split_meets_minimum_requirements(self, real_camera_geometry, valid_gcps_large):
    ...
    def test_validation_metrics_computed_for_train_and_test(self, real_camera_geometry, valid_gcps_large):
...
class TestSystematicErrorDetection:
    ...
    def test_detect_systematic_errors_directional_bias_detected(self):
        ...
        assert any('3.6' in w or '3.61' in w for w in warnings)  # magnitude = sqrt(3^2 + 2^2) = 3.606
...
class TestGenerateResidualPlot:
    ...
    def test_generate_residual_plot_basic_call(self, sample_calibration_result, sample_gcps_for_plot):
    ...
    def test_generate_residual_plot_with_validation_split(self, sample_calibration_result, sample_gcps_for_plot):
    ...
    def test_generate_residual_plot_saves_to_file(self, sample_calibration_result, sample_gcps_for_plot):
...
class TestGenerateResidualHistogram:
    ...
    def test_generate_residual_histogram_basic_call(self, sample_calibration_result):
        ...
        with patch('poc_homography.gcp_calibrator.plt') as mock_plt:
            ...
            fig = generate_residual_histogram(
                calibration_result=sample_calibration_result,
                bins=20
            )
...
class TestGenerateValidationReport:
    ...
    def test_generate_validation_report_includes_validation_status(self, sample_calibration_result):
        ...
        report = generate_validation_report(sample_calibration_result, validation_thresholds=thresholds)
...
class TestCalibrationValidationIntegration:
    ...
    def test_complete_validation_workflow(self, real_camera_geometry, valid_gcps_large):
        ...
        with patch('poc_homography.gcp_calibrator.plt') as mock_plt:
            ...
            fig_hist = generate_residual_histogram(
                calibration_result=result,
                bins=20
            )
