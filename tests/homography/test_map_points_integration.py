...
def valte_annotations():
    """Load Valte annotations from YAML file."""
...
@pytest.fixture
def homography(map_registry, valte_annotations):
    """Create and compute homography from test data."""
    h = MapPointHomography(map_id="test_map")
    # Convert Annotation objects to dict format for compute_from_gcps
    gcps_dicts = [
        {
            "pixel_x": ann.pixel.x,
            "pixel_y": ann.pixel.y,
...
class TestMapPointHomographyComputation:
    ...
    def test_compute_with_insufficient_gcps_raises(self, map_registry):
        ...
        h = MapPointHomography(map_id="test_map")
        gcps = [
            {"pixel_x": 800, "pixel_y": 580, "gcp_id": "PS1"},
            {"pixel_x": 1082, "pixel_y": 390, "gcp_id": "PS3"},
    ...
    def test_compute_with_missing_map_point_raises(self, map_registry):
        ...
        gcps = [
            {"pixel_x": 800, "pixel_y": 580, "gcp_id": "PS1"},
            {"pixel_x": 1082, "pixel_y": 390, "gcp_id": "PS3"},
            {"pixel_x": 408, "pixel_y": 776, "gcp_id": "NONEXISTENT"},
            {"pixel_x": 568, "pixel_y": 846, "gcp_id": "PS5"},
        ]
    ...
    def test_result_quality_metrics(self, homography):
        ...
        assert result.num_gcps == 13  # Number of annotations in test data
        ...
        assert result.mean_reproj_error < 20.0, "Mean error should be < 20 pixels"
...
class TestForwardProjection:
    ...
    def test_project_camera_center(self, homography):
        ...
        map_point = homography.camera_to_map(PixelPoint.create(960.0, 540.0))

        # Should return valid MapPoint with map pixel coordinates
        assert hasattr(map_point, "pixel_x")
        assert hasattr(map_point, "pixel_y")
        assert 700 < map_point.pixel_x < 1300, "X in expected range"
        assert 200 < map_point.pixel_y < 800, "Y in expected range"

    def test_project_gcp_points(self, homography, map_registry, valte_annotations):
        """Test projecting actual GCP points."""
        for ann in valte_annotations[:5]:  # Test first 5 annotations
            camera_pixel = PixelPoint.create(ann.pixel.x, ann.pixel.y)
            ...
            # Should be close (within 20 pixels)
            assert error < 20.0, f"Error for {ann.gcp_id}: {error:.2f} pixels"

    def test_batch_projection(self, homography, valte_annotations):
        """Test batch projection of multiple camera pixels."""
        camera_pixels = [PixelPoint.create(ann.pixel.x, ann.pixel.y) for ann in valte_annotations]
        ...
        for map_point in map_points:
            assert hasattr(map_point, "pixel_x")
            assert hasattr(map_point, "pixel_y")
            assert 700 < map_point.pixel_x < 1300
            assert 200 < map_point.pixel_y < 800
...
class TestInverseProjection:
    ...
    def test_project_map_point_to_camera(self, homography, map_registry):
        ...
        map_point = map_registry.points["PS1"]
        map_coord = PixelPoint.create(map_point.pixel_x, map_point.pixel_y)
    ...
    def test_inverse_projection_accuracy(self, homography, map_registry, valte_annotations):
        """Test that inverse projection is accurate."""
        errors = []

        for ann in valte_annotations[:10]:  # Test first 10
            # Get map coordinate
            ...
            expected_pixel = PixelPoint.create(ann.pixel.x, ann.pixel.y)
    ...
    def test_batch_inverse_projection(self, homography, map_registry, valte_annotations):
        """Test batch inverse projection."""
        # Get map coordinates for all annotations
        map_coords = []
        for ann in valte_annotations:
            map_point = map_registry.points[ann.gcp_id]
            map_coords.append(PixelPoint.create(map_point.pixel_x, map_point.pixel_y))
...
class TestRoundTripConsistency:
    ...
    def test_camera_to_map_to_camera(self, homography, valte_annotations):
        """Test camera -> map -> camera round-trip."""
        errors = []

        for ann in valte_annotations:
            original = PixelPoint.create(ann.pixel.x, ann.pixel.y)
    ...
    def test_map_to_camera_to_map(self, homography, map_registry, valte_annotations):
        """Test map -> camera -> map round-trip."""
        errors = []

        for ann in valte_annotations:
            map_point = map_registry.points[ann.gcp_id]
            original = PixelPoint.create(map_point.pixel_x, map_point.pixel_y)

            # Inverse then forward
            camera_pixel = homography.map_to_camera(original)
            recovered_point = homography.camera_to_map(camera_pixel)
            recovered = PixelPoint.create(recovered_point.pixel_x, recovered_point.pixel_y)
        ...
        # Round-trip errors in map pixels
        assert mean_error < 5.0, f"Mean round-trip error: {mean_error:.2f} pixels"
        assert max_error < 15.0, f"Max round-trip error: {max_error:.2f} pixels"
