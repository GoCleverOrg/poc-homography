...
from poc_homography.kml_generator import create_output_directory, generate_kml, haversine_distance
...
def test_generate_kml_creates_valid_xml(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_has_correct_structure(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_placemark_count(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_coordinates_format(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_with_empty_gcps(
    sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_missing_validation_details(sample_gcps, sample_camera_gps, temp_output_dir):
...
def test_generate_kml_invalid_camera_gps(sample_gcps, sample_validation_results, temp_output_dir):
...
def test_generate_kml_file_saved(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_kml_file_content_readable(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_kml_error_distance_accuracy(sample_gcps, temp_output_dir):
    ...
    validation_results = {
        "details": [{"projected_gps": (proj_lat, proj_lon), "error_meters": expected_error}]
    }
...
def test_kml_line_string_coordinates_match_points(
    sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
    ...
    for i, error_pm in enumerate(error_placemarks):
        ...
        assert line_points[0] == orig_coords, (
            "LineString first point should match original GCP coordinates"
        )
        assert line_points[1] == proj_coords, (
            "LineString second point should match projected GCP coordinates"
        )
...
def test_generate_kml_with_many_gcps(sample_camera_gps, temp_output_dir):
    ...
    for i in range(num_gcps):
        ...
        validation_results["details"].append(
            {"projected_gps": (proj_lat, proj_lon), "error_meters": error}
        )
