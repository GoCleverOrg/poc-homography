...
def quick_test():
    ...
    det_H_inv = np.linalg.det(H_inv)
    identity_test = np.allclose(H @ H_inv, np.eye(3), atol=1e-6)
    ...
    print("\n  Checks:")
    for desc, check_result, symbol in checks:
        print(f"    {symbol} {desc}: {check_result}")
