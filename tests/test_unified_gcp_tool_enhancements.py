...
class TestCornerExtraction:
    ...
    def mock_session(self):
        ...
        session.extract_corner_points_from_mask = (
            UnifiedSession.extract_corner_points_from_mask.__get__(session, UnifiedSession)
        )
...
class TestEdgeDistanceScore:
    ...
    def mock_session(self):
        ...
        session = Mock(spec=UnifiedSession)
        session._calculate_edge_distance_score = (
            UnifiedSession._calculate_edge_distance_score.__get__(session, UnifiedSession)
        )
...
class TestSpatialDistributionPenalty:
    ...
    def mock_session(self):
        ...
        session = Mock(spec=UnifiedSession)
        session._calculate_spatial_distribution_penalty = (
            UnifiedSession._calculate_spatial_distribution_penalty.__get__(session, UnifiedSession)
        )
...
class TestSpatialDistributionMetrics:
    ...
    def mock_session(self):
        ...
        session = Mock(spec=UnifiedSession)
        session.calculate_spatial_distribution = (
            UnifiedSession.calculate_spatial_distribution.__get__(session, UnifiedSession)
        )
...
class TestAutoSuggestIntegration:
    ...
    def mock_session_full(self):
        ...
        session.auto_suggest_gcp_points = UnifiedSession.auto_suggest_gcp_points.__get__(
    ...
    def test_respects_max_suggestions(self, mock_session_full):
        ...
        for i in range(5):
            for j in range(5):
                cv2.rectangle(
                    mask, (i * 100 + 10, j * 100 + 10), (i * 100 + 90, j * 100 + 90), 255, -1
                )
