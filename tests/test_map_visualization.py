...
from poc_homography.map_debug_server import find_available_port, generate_html, start_server
...
def test_generate_html_contains_required_elements(
    sample_gcps, sample_validation_results, sample_camera_gps
):
...
def test_generate_html_gcps_embedded(sample_gcps, sample_validation_results, sample_camera_gps):
...
def test_generate_html_camera_gps_centered(
    sample_gcps, sample_validation_results, sample_camera_gps
):
...
def test_generate_html_google_maps_without_api_key(
    sample_gcps, sample_validation_results, sample_camera_gps
):
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        assert "google.addTo(map)" in html, "Google should be default layer even without API key"


def test_generate_html_google_maps_with_api_key(
    sample_gcps, sample_validation_results, sample_camera_gps
):
    ...
    with patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": test_api_key}):
        ...
        assert "google.addTo(map)" in html, "Google should be default layer with API key"


def test_generate_html_valid_structure(sample_gcps, sample_validation_results, sample_camera_gps):
    ...
    assert html.strip().startswith("<!DOCTYPE html>"), "HTML should start with DOCTYPE declaration"
...
def test_server_output_directory_created(
    sample_gcps, sample_validation_results, sample_camera_gps, sample_frame, temp_output_dir
):
...
def test_export_frame_with_markers_creates_file(
    sample_frame, sample_gcps, sample_validation_results, temp_output_dir
):
...
def test_export_frame_with_markers_draws_elements(
    sample_gcps, sample_validation_results, temp_output_dir
):
    ...
    assert non_white_pixels > 0, "Exported frame should have non-white pixels (markers drawn)"
    ...
    for gcp in sample_gcps:
        ...
        if 0 <= v < 1080 and 0 <= u < 1920:
            region = marked_frame[
                max(0, v - 10) : min(1080, v + 10), max(0, u - 10) : min(1920, u + 10)
            ]

            # Region should have some non-white pixels
            region_non_white = np.sum(np.any(region != 255, axis=2))
            assert region_non_white > 0, f"Region around GCP at ({u}, {v}) should have markers"
...
def test_integration_end_to_end(
    sample_frame, sample_gcps, sample_validation_results, sample_camera_gps, temp_output_dir
):
...
def test_generate_html_with_empty_validation_results(sample_gcps, sample_camera_gps):
    ...
    for gcp in sample_gcps:
        lat = gcp["gps"]["latitude"]
        assert str(lat) in html, "GCP data should be in HTML even without validation"
