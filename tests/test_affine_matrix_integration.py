...
class TestAffineMatrixIntegration:
    ...
    def test_backward_compatibility_default_a_matrix(self):
        ...
        np.testing.assert_array_almost_equal(
            H_total, geo.H, decimal=10,
            err_msg="H_total should equal H when A is identity"
        )

        # Verify this is mathematically equivalent (H @ I = H)
        assert np.allclose(H_total, geo.H), \
            "Matrix multiplication H @ I should preserve H"
    ...
    def test_homography_composition_correct(self):
        ...
        assert abs(det_H_total) > 1e-10, \
            "H_total should be invertible (non-zero determinant)"

        cond_H_total = np.linalg.cond(H_total)
        assert cond_H_total < 1e10, \
        ...
        np.testing.assert_almost_equal(world_x, expected_world_x, decimal=6,
            err_msg="World X coordinate should match expected transformation")
        np.testing.assert_almost_equal(world_y, expected_world_y, decimal=6,
        ...
        np.testing.assert_array_almost_equal(
            camera_point_total, camera_point_composed, decimal=10,
    ...
    def test_equivalence_with_inline_computation(self):
        ...
        np.testing.assert_allclose(
            A_new, T_pixel_to_localXY, rtol=1e-15, atol=1e-15,
    ...
    def test_numerical_precision_large_utm(self):
        ...
        np.testing.assert_allclose(
            geo.A, expected_A, rtol=1e-12, atol=1e-12,
        ...
        assert abs(geo.A[0, 2] - t_x) < 1e-10, \
            "Translation t_x should be computed precisely"
        assert abs(geo.A[1, 2] - t_y) < 1e-10, \
            "Translation t_y should be computed precisely"
        ...
        np.testing.assert_almost_equal(
            actual_diff, expected_diff, decimal=10,
    ...
    def test_a_matrix_reset_to_identity(self):
        ...
        assert np.allclose(geo.A, np.eye(3)), \
            "A should be identity on initialization"
        ...
        np.testing.assert_array_equal(
            geo.A, np.eye(3),
            err_msg="A should reset to identity when geotiff_params is None"
        )
        ...
        np.testing.assert_array_equal(
            geo.A, np.eye(3),
            err_msg="A should reset to identity when camera_utm_position is None"
        )
...
class TestAffineMatrixCompositionEdgeCases:
    ...
    def test_composition_with_identity_a(self):
        ...
        np.testing.assert_allclose(
            H_total, geo.H, rtol=1e-15, atol=1e-15,
            err_msg="H @ I should equal H exactly"
        )
    ...
    def test_composition_associativity(self):
        ...
        np.testing.assert_allclose(
            result1, result2, rtol=1e-15, atol=1e-15,
    ...
    def test_composition_preserves_homogeneous_scaling(self):
        ...
        np.testing.assert_allclose(
            result1_norm, result2_norm, rtol=1e-10, atol=1e-10,
