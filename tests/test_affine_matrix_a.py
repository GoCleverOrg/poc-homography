...
class TestAffineMatrixInitialization:
    ...
    def test_a_matrix_defaults_to_identity(self):
        ...
        geo = CameraGeometry(1920, 1080)
        expected = np.eye(3)
        np.testing.assert_array_equal(geo.A, expected,
            err_msg="A matrix should default to identity matrix")
...
class TestSetGeotiffParamsValidation:
    ...
    def test_none_geotiff_params_sets_identity(self):
        ...
        expected = np.eye(3)
        np.testing.assert_array_equal(geo.A, expected,
            err_msg="A should be identity when geotiff_params is None")
    ...
    def test_none_camera_utm_position_sets_identity(self):
        ...
        expected = np.eye(3)
        np.testing.assert_array_equal(geo.A, expected,
            err_msg="A should be identity when camera_utm_position is None")
    ...
    def test_both_none_sets_identity(self):
        ...
        expected = np.eye(3)
        np.testing.assert_array_equal(geo.A, expected,
            err_msg="A should be identity when both params are None")
...
class TestSetGeotiffParamsLogging:
    ...
    def test_logs_a_matrix_computation(self, mock_logger):
        ...
        assert any('A matrix' in str(call) or 'affine' in str(call).lower()
...
class TestAffineMatrixDocumentation:
    ...
    def test_docstring_mentions_coordinate_system(self):
        ...
        assert any(keyword in docstring for keyword in
                   ['coordinate', 'transformation', 'affine', 'geotiff', 'utm']), \
