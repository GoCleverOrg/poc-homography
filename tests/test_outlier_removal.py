...
class TestRemoveOutliersConsistency(unittest.TestCase):
    ...
    def test_remove_outliers_removes_exactly_ransac_outliers(self):
        ...
        self.session.gcps = [{"metadata": {"description": f"GCP {i}"}} for i in range(7)]
    ...
    def test_remove_outliers_preserves_inliers(self):
        ...
        self.session.gcps = [{"metadata": {"description": f"GCP {i}"}} for i in range(5)]
        ...
        remaining_descriptions = [g["metadata"]["description"] for g in self.session.gcps]
