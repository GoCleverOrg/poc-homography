...
def test_tilt_inversion(pan_deg, tilt_deg, height=5.0):
    ...
    print("="*70)
    print(f"TESTING TILT SIGN CONVENTION")
    print("="*70)
    print(f"\nCamera parameters:")
    ...
    for sign, desc in [(1, "AS REPORTED"), (-1, "INVERTED")]:
        ...
        if abs(pt_world[2, 0]) < 1e-6:
            print(f"  Image center: INVALID (near horizon)")
            continue
        ...
        print(f"\n  Sanity Checks:")
        if Yw > 0:
            print(f"    ✓ Y > 0: Point is AHEAD of camera")
        else:
            print(f"    ✗ Y < 0: Point is BEHIND camera (WRONG!)")
        ...
        if abs(pt_bottom[2,0]) > 1e-6 and abs(pt_top[2,0]) > 1e-6:
            dist_bottom = np.sqrt((pt_bottom[0,0]/pt_bottom[2,0])**2 + (pt_bottom[1,0]/pt_bottom[2,0])**2)
            dist_top = np.sqrt((pt_top[0,0]/pt_top[2,0])**2 + (pt_top[1,0]/pt_top[2,0])**2)

            print(f"    Bottom of image distance: {dist_bottom:.2f}m")
            print(f"    Top of image distance: {dist_top:.2f}m")

            if dist_bottom < dist_top:
                print(f"    ✓ Bottom < Top: Correct (bottom is closer)")
            else:
                print(f"    ✗ Bottom > Top: WRONG (bottom should be closer)")
