...
class IntrinsicExtrinsicHomography(GPSPositionMixin, HomographyProviderExtended):
    ...
    Attributes:
        ...
        _camera_gps_lat: Camera GPS latitude (initialized by GPSPositionMixin)
        _camera_gps_lon: Camera GPS longitude (initialized by GPSPositionMixin)
    """

    # Earth radius for GPS conversion (meters) - approximate for equirectangular projection
    EARTH_RADIUS = 6371000.0

    # Meters per degree of latitude (approximate for equirectangular projection)
    # This is an average value: 1 degree of latitude ≈ 111,111 meters
    # For more accurate conversions, use proper geodetic transformations
    ...
    def __init__(
        ...
        **kwargs  # Accept and ignore other kwargs for forward compatibility
        ...
        """
        # Call parent initializers to ensure proper MRO initialization
        ...
        # Note: _camera_gps_lat and _camera_gps_lon are now initialized by GPSPositionMixin
    ...
    def _calculate_ground_homography(
        ...
        if abs(H[2, 2]) < self.MIN_DET_THRESHOLD:
            ...
            logger.warning(
                "Homography normalization failed: H[2,2] = %.2e is below threshold %.2e. "
                "Returning identity matrix. This typically indicates invalid camera parameters "
                "or extreme viewing geometry.",
                H[2, 2],
                self.MIN_DET_THRESHOLD
            )
    ...
    def _local_to_gps(self, x_meters: float, y_meters: float) -> Tuple[float, float]:
        ...
        Raises:
            ...
            ValueError: If camera is at or very near poles (±89.9°)
        """
        ...
        # Check if camera is at or near poles (cos(90°) = 0, causing division by zero)
        POLE_THRESHOLD = 89.9
        if abs(self._camera_gps_lat) >= POLE_THRESHOLD:
            raise ValueError(
                f"Camera latitude ({self._camera_gps_lat}°) is at or very near poles "
                f"(threshold: ±{POLE_THRESHOLD}°). GPS conversion using equirectangular "
                f"projection is undefined at poles due to division by zero in longitude "
                f"calculation (cos(latitude) ≈ 0)."
            )

        # Convert meters to degrees
        # Latitude: 1 degree ≈ 111,111 meters
        # Longitude: 1 degree ≈ 111,111 * cos(latitude) meters
        lat_deg = y_meters / self.METERS_PER_DEGREE

        # Adjust longitude for latitude
        lat_rad = math.radians(self._camera_gps_lat)
        lon_deg = x_meters / (self.METERS_PER_DEGREE * math.cos(lat_rad))
    ...
    def compute_homography(
        ...
        if camera_position[2] <= 0:
            raise ValueError(
                f"Camera height (Z={camera_position[2]}) must be positive for ground plane "
                f"homography. A camera at or below ground level (Z <= 0) makes the homography "
                f"mathematically invalid and undefined."
            )
    ...
    def get_pixels_per_meter(self) -> float:
        """
        Get the scale factor for map visualization.

        Returns:
            float: Scale factor in pixels per meter (default: 100.0)
        """
    ...
    # =========================================================================
    # NOTE: The following methods are maintained for backward compatibility
    # with the camera_geometry.py interface. They duplicate functionality
    # available through the HomographyProviderExtended interface.
    ...
    def project_image_to_map(
        ...
        .. deprecated::
            ...
            Use :meth:`project_points_to_map` instead, which returns MapCoordinate
            objects with metric coordinates and confidence scores. For pixel
            coordinates, convert the MapCoordinate results using _world_to_map_pixels().
        ...
        """
        warnings.warn(
            "project_image_to_map() is deprecated. Use project_points_to_map() instead, "
            "which returns MapCoordinate objects with metric coordinates and confidence scores.",
            DeprecationWarning,
            stacklevel=2
        )
        if not self.is_valid():
    ...
    def world_to_map(
        ...
        .. deprecated::
            This method is deprecated and maintained only for backward compatibility.
            This method provides a low-level conversion from metric world coordinates
            to pixel coordinates. Use _world_to_map_pixels() for internal conversions,
            or work with MapCoordinate objects from project_point_to_map() which
            provide metric coordinates with confidence scores.
        ...
        """
        warnings.warn(
            "world_to_map() is deprecated. Use _world_to_map_pixels() for internal use, "
            "or work with MapCoordinate objects from project_point_to_map().",
            DeprecationWarning,
            stacklevel=2
        )
        if sw is None:
