...
class HomographyConfig:
    ...
    def save_to_yaml(self, path: str) -> None:
        ...
        - Resolves the path to its absolute canonical form (handles symlinks)
        ...
        Args:
            path: Path where configuration file should be written.
                Can be a relative path (resolved relative to current working directory)
                or an absolute path. The resolved path must be within the project
                directory or its subdirectories.

        Raises:
            IOError: If file cannot be written
            ValueError: If the resolved path escapes the project directory boundary

        Example:
            >>> config = get_default_config()
            >>> config.save_to_yaml('my_config.yaml')
            >>> config.save_to_yaml('configs/production.yaml')
        """
        # Convert to Path object and resolve to absolute canonical path
        # This handles '.', '..', symlinks, and other path manipulations
        config_path = Path(path).resolve()

        # Get the project root directory (current working directory resolved)
        # This serves as our security boundary
        project_root = Path.cwd().resolve()

        # Validate that the resolved path is within the project directory
        # is_relative_to() returns True if config_path is under project_root
        try:
            # Check if resolved path is within project boundaries
            # This prevents path traversal attacks like '../../../etc/passwd'
            config_path.relative_to(project_root)
        except ValueError:
            # Path is outside project directory - security violation
            raise ValueError(
                f"Security: Path '{path}' resolves to '{config_path}' which is outside "
                f"the project directory '{project_root}'.\n"
                f"Configuration files must be written within the project directory. "
                f"Attempted path traversal detected."
            ) from None

        # Create parent directories if needed (now safe, as path is validated)
